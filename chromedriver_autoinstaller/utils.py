...
def webdriver_exist(path:str=None):
    ...
    if not os.path.isfile(chromedriver_filepath) or \
        ...
        return (False,None)
    return (True,chromedriver_filepath)


def download_chromedriver(path:str=None, handler=None):
    """
    ...
    chromedriver_filename = get_chromedriver_filename()
    chromedriver_filepath = os.path.join(chromedriver_dir, chromedriver_filename)
    chromedriver_filepath_zip = os.path.join(chromedriver_dir, "chromedriver.zip")
    if not os.path.isfile(chromedriver_filepath) or \
            not check_version(chromedriver_filepath, chromedriver_version):
        ...
        try:
            archive, header = urllib.request.urlretrieve(url, chromedriver_filepath_zip, handler)
